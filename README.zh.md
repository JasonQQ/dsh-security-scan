...
| | 规则数 | 覆盖方向 |
| --- | ---: | --- |
| 静态体检——行规则 | 61 | 凭据读取、网络回调、混淆、安装脚本、供应链、滥用宿主机、持久化、提权、外传、提示注入、破坏性 |
| 静态体检——关联规则 | 18 | 包级关联：凭据读取 **且** 有出站、混淆 **且** 有回调、安装钩子 **且** 有网络 |
...
**160 条规则**全部带中文文案，按 rule id 分别放在两份 `rules.zh.ts` 里。之后新增而尚未翻译的规则会回退成英文而不是被省略，覆盖率由 `/security status` 报出——缺口表现为一个数字，而不是半份悄悄少了一种语言的报告。
...
- **默认阈值还不是可在生态规模上使用的拒绝线。** 对市场中按 stars 排名前 100 的插件做批量体检（见 `docs/stress/`），能够扫描的 91 个里拒绝了 67 个（74%）。读这些报告时发现九条规则是靠**不支持其结论的证据**决定评级的——一条 UI 文案被当成凭据读取、bundler 自己的加载器被当成混淆载荷、断言清洗器生效的测试被当成发布的破坏性命令——修掉之后这次压测从 80 个拒绝降到 67 个。这是进展而不是收敛：91 个里 67 个仍然是一个会被直接关掉的闸门，而且一半报告恰好是 `0/100`，分数也还不构成梯度。已修好的、有实测数据的、以及仍然open的问题都写在 [`docs/stress/CALIBRATION.md`](docs/stress/CALIBRATION.md)。把 `enforce` 指向整个生态之前，先用 `monitor` 跑一轮并读日志。
...
npm run typecheck
npm run build
npm test              # 233 个单元 + 集成测试，之后跑对抗性冒烟检查
